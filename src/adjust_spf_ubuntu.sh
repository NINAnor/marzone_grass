...
#Lock to open montado areas?
		...
		cp "${mfolder}/input_backup/pulock_om_pa.dat" "${mfolder}/${input_folder}/pulock.dat"
	#	r.stats -1 -n --verbose input=PU,SPA_SAC_rel_200m separator="," | sort -n -t "," >> "${mfolder}/${input_folder}/pulock_pre1.dat"
	fi
	...
	do
		...
		if [ $b -eq 0 ] ; then
			cat "${mfolder}/input_backup.dat" | sed '/BLM/c\BLM 0' > "${mfolder}/input.dat.pre"
			cat "${mfolder}/input.dat.pre" | sed "s/BOUNDNAME/#BOUNDNAME/" > "${mfolder}/input.dat"
		else
			cat "${mfolder}/input_backup.dat" | sed '/BLM/c\BLM 1' > "${mfolder}/input.dat"
			cat "${mfolder}/input_backup/zoneboundcost.dat"  | sed "/1,2,1.0/c\1,2,${b}.0" > "${mfolder}/${input_folder}/zoneboundcost.dat"
		fi
		...
		do
			...
			do
				...
				#30% target for hieraaetus cannot be met in open montado and protected areas only (lowering it to 25%)
				if [ "$st"="0.3" -a "$l"!="all" ] ; then
					sed -i 's/18,0.3,/18,0.2,/' "${mfolder}/${input_folder}/feat.dat"
				fi
				...
				cat output/openmontado_ssoln.txt | sort -n -k1 -t',' | awk -v FS="," '{if(NR>=2) print $1, "=", $4}' | r.reclass --o input=PU output=ssoln_AEM_eligible_${l}_BLM${b}_ht${ht}_st${st} rules=- --o --v
